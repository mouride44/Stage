...
def open_csv(fichier):
    df=pd.read_csv(fichier,skiprows=1,
...
def check_table_my(cursor,tablename):
    ...
    cursor.close()
    return False 
def recupere_espece(cursor,maxid):
    cursor.execute("SELECT espece  FROM lait WHERE indexes=%s", (maxid,))
